...
<div class="content">
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Detail Materi <?= $pertemuan['judul_pertemuan'] ?>
        </h2>
    </div>
            ...
            <div class="blog__preview image-fit">
                ...
                <div class="absolute w-full flex items-center px-5 pt-6 z-10">
                    <div class="w-10 h-10 flex-none image-fit">
                        <img alt="Midone Tailwind HTML Admin Template" class="rounded-full" src="<?= base_url('public/profiel/') . $pertemuan['foto'] ?>">
                    </div>
                    <div class="ml-3 text-white mr-auto">
                        <a href="" class="font-medium"><?= $pertemuan['nama']  ?></a>
                        <div class="text-xs"><?= $pertemuan['tanggal_pertemuan'] ?></div>
                    </div>
                </div>
                <div class="absolute bottom-0 text-white px-5 pb-6 z-10"> <span class="blog__category px-2 py-1 rounded">nama mapel</span>
                    <p class="block font-medium text-xl mt-3"><?= $pertemuan['judul_pertemuan'] ?></p>
                </div>
            </div>
            <div class="p-5 text-gray-700"><?= $pertemuan['materi'] ?></div>
            <div class="flex items-center px-5 py-3 border-t border-gray-200">
                <?php if ($pertemuan['file'] != 'tidak ada') : ?>
                    <a href="<?= base_url('user/download/materi/') . $pertemuan['file'] ?>" class="intro-x w-8 h-8 flex items-center justify-center rounded-full bg-theme-1 text-white ml-2 tooltip" title="Download Materi"> <i data-feather="share" class="w-3 h-3"></i> </a>
                <?php endif; ?>
            </div>
...
<?php foreach ($komentar as $k) : ?>
    <div class="modal" id="komentar<?= $k['id_km'] ?>">
        <div class="modal__content">
            ...
            <form action="<?= base_url('siswa/balasKomentar') ?>" method="post" enctype="multipart/form-data">
            <input type="hidden" name="foto" value="<?= $profiel['foto'] ?>">
                            <input type="hidden" name="nama" value="<?= $profiel['nama'] ?>">
                            <input type="hidden" name="id_pertemuan" value="<?= $pertemuan['id_pertemuan'] ?>">
